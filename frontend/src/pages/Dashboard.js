...
/**
 * FIXES:
 * 1. Patients and appointments always loaded from DB — no sample data fallback
 *    that was hiding real errors and showing fake patients on dashboard.
 * 2. bloodGroup rendered correctly via resolveBloodGroup() (backend returns enum object).
 * 3. appointmentTime shown correctly from DB (may be "09:30:00" — display as-is or trim).
 * 4. Dashboard stats computed from real data.
 * 5. Sample bed/trend data kept only for analytics charts (those endpoints may not exist).
 */

const COLORS = ['#00d4ff', '#00e5a0', '#f59e0b', '#f43f5e', '#a78bfa', '#fb923c'];

const SAMPLE_BED_DATA = [
  { department: 'Cardiology',  totalBeds: 30, occupiedBeds: 22, availableBeds: 8  },
  { department: 'Neurology',   totalBeds: 20, occupiedBeds: 15, availableBeds: 5  },
  { department: 'Pediatrics',  totalBeds: 25, occupiedBeds: 12, availableBeds: 13 },
  { department: 'Orthopedics', totalBeds: 35, occupiedBeds: 28, availableBeds: 7  },
  { department: 'ICU',         totalBeds: 15, occupiedBeds: 12, availableBeds: 3  },
  { department: 'Emergency',   totalBeds: 20, occupiedBeds: 16, availableBeds: 4  },
];
const SAMPLE_TREND = [
...
/** Format "09:30:00" or "09:30" to "09:30" */
const formatTime = (t) => {
  if (!t) return '—';
...
export default function Dashboard() {
  ...
  const loadData = useCallback(async (silent = false) => {
    ...
    try {
      // ── Real DB data — no sample fallback for patients/appointments ──
      let pats = [], appts = [], recs = [];
      let beds = SAMPLE_BED_DATA, trend = SAMPLE_TREND;

      try {
        const r = await patientAPI.getAll();
        pats = r.data || [];
      } catch (e) {
      ...
      try {
        ...
        recs = r.data || [];
      } catch { /* medical records optional */ }

      try {
        const r = await analyticsAPI.getBedOccupancy();
        if (r.data && r.data.length > 0) beds = r.data;
      } catch { /* use sample bed data */ }

      try {
        const r = await analyticsAPI.getTrend();
        if (r.data && r.data.length > 0) trend = r.data;
      } catch { /* use sample trend */ }
      ...
      // Show 5 most recent patients (list is already ordered by createdAt desc from backend)
      setPatients(pats.slice(0, 5));
      // Show today's appointments (up to 5)
  ...
  return (
    <div className="dashboard">
            ...
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={bedData} margin={{ top: 0, right: 0, bottom: 0, left: -20 }}>
                ...
                <XAxis dataKey="department" tick={{ fill: 'var(--text-muted)', fontSize: 10 }}
                  tickLine={false} axisLine={false} tickFormatter={v => v.substring(0, 5)} />
      ...
      <div className="dash-tables-row">
          ...
          <div className="table-wrapper">
            {patients.length === 0 ? (
              <div className="empty-state" style={{ padding: '40px' }}>
                ...
                <button className="btn btn-primary btn-sm" onClick={() => setShowPatientForm(true)}>
              ...
              <table className="data-table">
                <thead>
                  <tr><th>Patient</th><th>ID</th><th>Blood</th><th>Doctor</th><th>Status</th><th></th></tr>
                </thead>
                <tbody>
                  {patients.map(p => {
                    const bloodGroup = resolveBloodGroup(p.bloodGroup);
                    return (
                      <tr key={p.id}>
                        <td>
                          <div className="info-cell">
                            <div className="avatar avatar-sm"
                              style={{ background: p.gender === 'FEMALE' ? '#f43f5e' : '#00d4ff' }}>
                              {p.firstName?.charAt(0)}{p.lastName?.charAt(0)}
                            </div>
                            <div>
                              <div className="info-name">{p.firstName} {p.lastName}</div>
                              <div className="info-sub">{p.city || p.mobile || '—'}</div>
                            </div>
                          </div>
                        </td>
                        <td><span className="text-muted fs-12">{p.patientId || `#${p.id}`}</span></td>
                        <td>
                          {bloodGroup
                            ? <span className="blood-badge">{bloodGroup}</span>
                            : <span className="text-muted">—</span>
                          }
                        </td>
                        <td><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{p.primaryDoctorName || '—'}</span></td>
                        <td>
                          <span className={`badge badge-${p.patientStatus === 'ACTIVE' ? 'success' : 'muted'}`}>
        ...
        {/* Today's Appointments — real DB data */}
        <div className="card">
          <div className="card-header">
          ...
          <div className="table-wrapper">
            {appointments.length === 0 ? (
              <div className="empty-state" style={{ padding: '40px' }}>
                ...
                <button className="btn btn-primary btn-sm" onClick={() => setShowAppointmentForm(true)}>
                  + Book Appointment
                </button>
              </div>
            ) : (
                    ...
                    <tr key={a.id}>
                      <td>
                        <div>
                          ...
                          <div className="info-sub">{a.department || '—'}</div>
                        </div>
                      </td>
                      <td><span style={{ fontSize: 12, color: 'var(--text-secondary)' }}>{a.doctorName || '—'}</span></td>
                      <td>
                      ...
                      <td>
                        <span className={`badge badge-${STATUS_BADGE[a.status] || 'muted'}`}>{a.status}</span>
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      {showPatientForm && (
        <PatientForm
          ...
          onSaved={() => { setShowPatientForm(false); loadData(true); toast.success('Patient registered!'); }}
        />
      )}
      {showAppointmentForm && (
        <AppointmentForm
          onClose={() => setShowAppointmentForm(false)}
          onSaved={() => { setShowAppointmentForm(false); loadData(true); toast.success('Appointment booked!'); }}
        />
      )}
    </div>
  );
